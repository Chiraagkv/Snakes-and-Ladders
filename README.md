...
* **Persistent Saves:** Allows saving and loading game state to/from s\_and\_l\_save.txt. Automatically prompts to resume a previous game on startup.
...
| include/s\_and\_l.h | Defines all global constants, data structures (Player, Snake, Ladder), external variables, and function prototypes. |
...
| Command | Action |
| :---- | :---- |
| make | **Builds the executable.** Compiles all .c files in src/, creates the necessary build/ directory, and links the final executable named snakes\_and\_ladders. |
...
| make clean | **Cleans up.** Removes the build/ directory, the snakes\_and\_ladders executable, and any created save file (s\_and\_l\_save.txt). |
